function [PONTOS,simetria] = find_rectangles_paa(img_original, dT, H, rho, theta, pico, pic, pos_xy)
...
%   rho_and_theta = [rho;  180/pi*theta'; pic'],
 ...
 % schramm: rever esses parametros
    L_theta_intra = max(3,5*180*dT/pi);  %limiar do angulo para o mesmo par
    L_theta_inter = max(3,5*180*dT/pi);   %limiar do angulo entre dois pares
%    L_rho = 3;   %limiar da distancia 
%    L_pico = 35; %limiar de tamanho dos lados dos retangulos (em %)
%     L_comp=.5;
...
%    L_theta_inter = 3;   %limiar do angulo entre dois pares
    ...
    L_comp=1; % limiar da altura dos picos com relacao ao lado esperado
    ...
    for index1=1:length(rho);
        for index2=(index1+1):length(rho);
            ...
            % teste
            %
            if diferenca_theta>180-L_theta_intra;
                ang(index1)=-ang(index1);
                rho(index1)=-rho(index1);
                diferenca_theta=L_theta_intra/2;
            end,
            %
            %
            % fim teste
            ...
            if (diferenca_theta<=L_theta_intra)
                ...
                if (diferenca_rho<=L_rho) & (diferenca_pico<=(pic(index1)+pic(index2))*L_pico/200),
                    cont = cont+1;
                    %
...
%plot(centroy,centrox,'w.');pause(.0001);
