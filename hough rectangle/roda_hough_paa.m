function [H, berough, rho, theta, pico, valor_pico]=roda_hough_paa(mag,dT,dS,RMin)
%
...
%    load mask;hh=9;w=5;
%    hh=9;w=7;
    hh=7;w=round(20*pi/180/dT);  % pega uma janela de +- 5 graus em torno de cada ponto
    mask=ones(hh,w)/hh/w;
...
%        load lixo;save lixo WIN h berough
    ...
    Tn=.1;
    szey=round(20*pi/180/dT); % maximo em uma janela de 10 graus
    szex=2*RMin;
...
%    H = (H==mx)&(H>Tn)&(h>1.2*RMin)&(be>3*RMin);  
    H = (H==mx)&(H>Tn)&(h>=1.5*RMin)&(be>3*RMin);
...
%    sum(H(:))
    ...
    %
    Max_picos=10;  %schramm (ver extracao e altura picos)
    lista=berough(H>0);
%    lista=h(H>0);
if ~isempty(lista),
        lista=sort(lista);
...
%        H(h<=thresh)=0;
        ...
        for i=1:length(rho_ind),
            valor_pico=[valor_pico; sum(h(max(rho_ind(i)-1,1):min(rho_ind(i)+1,s(1)),theta_ind(i)))];
        end,
%valor_pico=h(H>0);
% end,
else,
