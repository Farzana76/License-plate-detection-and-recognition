function [retangulos_new, centros_new]=remove_duplicated_rectangles2(img, retangulos, centros, indice, simetry);
%
...
%
% plota o resultados final
%
ss=size(retangulos_new);ss=[ss 1]; % aumenta o valor de ss para evitar erros no caso de um retangulo
figure,imshow(img);
hold on;for i=1:ss(3);
    xx=retangulos_new(:,:,i);
    centro=centros_new(i,:);
    line(xx(2,:),xx(1,:),'LineWidth',2,'Color','r'); axis ij; hold on;
%    plot(xx(2,:),xx(1,:),'g'); axis ij; 
